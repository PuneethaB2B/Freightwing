...
report 50010 "Update Data"
{
    ...
    var
        ...
        gRecEmailBody2: Record "Email Body";
        gRecParameters1: Record 50156;
        gRecParameters2: Record "Report Request Parameters";
        gRecMAWBLineChrg1: Record 50152;
        gRecMAWBLineChrg2: Record "MAWB Line Charge";
        gRecMAWBBaseChrg1: Record 50151;
        gRecMAWBBaseChrg2: Record "MAWB Base Charge";
        gRecUnrecoveredChrg1: Record 50150;
        gRecUnrecoveredChrg2: Record "Unrecovered Charge";
        gRecImportCharge1: Record 50149;
        gRecImportCharge2: Record "Import Charge";
        gRecShipmentUpdateLine1: Record 50148;
        gRecShipmentUpdateLine2: Record "Shipment Update Line";
        gRecImportActivities1: Record 50147;
        gRecImportActivities2: Record "Import Activities";
        gRecIATAPurchaseInv1: Record 50140;
        gRecIATAPurchaseInv2: Record "IATA Purchase Invoice";
        gRecBeneficiaryDetails1: Record 50103;
        gRecBeneficiaryDetails2: Record "Beneficiary Details";
        gRecPreAlertLine1: Record 50101;
        gRecPreAlertLine2: Record "Pre Alert Line";
        gRecPreAlertHeader1: Record 50100;
        gRecPreAlertHeader2: Record "Pre Alert Header";
        gRecHandlingSlabMatrix1: Record 50099;
        gRecHandlingSlabMatrix2: Record "Handling Slab Matrix";
        gRecGoodReceiptULDAllocation1: Record 50098;
        gRecGoodReceiptULDAllocation2: Record "Good Receipt ULD Allocation";
        gRecGoodReceiptNotifyParty1: Record 50097;
        gRecGoodReceiptNotifyParty2: Record "Good Receipt Notify Party";
        gRecGoodReceiptLine1: Record 50096;
        gRecGoodReceiptLine2: Record "Good Receipt Line";
        gRecGoodReceiptHeader1: Record 50095;
        gRecGoodReceiptHeader2: Record "Good Receipt Header";
        gRecDumpCustEnt1: Record 50092;
        gRecDumpCustEnt2: Record "Dump Cust Ent";
        gRecShipperConsignee1: Record 50091;
        gRecShipperConsignee2: Record "Shipper Consignee";
        gRecShipperNotifyParty1: Record 50090;
        gRecShipperNotifyParty2: Record "Shipper Notify Party";
        gRecMAWBInvoiceNotifyParty1: Record 50087;
        gRecMAWBInvoiceNotifyParty2: Record "MAWB Invoice Notify Party";
        gRecMAWBInvoiceCharge1: Record 50086;
        gRecMAWBInvoiceCharge2: Record "MAWB Invoice Charge";
        gRecHAWBLine1: Record 50083;
        gRecHAWBLine2: Record "HAWB Line";
        gRecHAWBHeader1: Record "HS Codes";
        gRecHAWBHeader2: Record "HAWB Header";
}

 */ //B2BUPG this report is not compiled in client version