report 50123 "Statement_UPG"
{
    ...
    begin
        GetToAddressFromCustomer(Customer."No.");
    end;
    ...
    local procedure GetToAddressFromCustomer(BillToCustomerNo: Code[20]): Text[250]
    var
        CustomerRec: Record Customer;
        ToAddress: Text;
        ReportParameters: Record "Report Request Parameters";
    begin
        IF CustomerRec.GET(BillToCustomerNo) THEN
            ToAddress := CustomerRec."E-Mail";

        IF ReportParameters.GET(116, USERID) THEN BEGIN
            ReportParameters.ToEmail := ToAddress;
            ReportParameters."Statement Period Start Date" := StartDate;
            ReportParameters."Statement Period End Date" := EndDate;
            ReportParameters.MODIFY;
        END;
    end;
}
